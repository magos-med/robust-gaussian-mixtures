...
class ChoquetGaussianMixture(StandardGaussianMixture):
    name = 'choquet'
    empiric_samples = 100

    def find_means(self, x, resp, nk):
        means = []
        for k in range(len(nk)):
            x_new = x.T * resp[:, k]
            x_new = x_new.T[~(abs(x_new[0, :]) <= 1e-10)]
            if len(x_new) == 0:
                means.append([0] * x_new.shape[1])
            else:
                mean = np.apply_along_axis(choquet_mean, 0, x_new, n=self.empiric_samples)
                means.append(mean)

        return np.array(means)


class OrtizChoquetGaussianMixture(OrtizGaussianMixture, ChoquetGaussianMixture):
    name = 'choquet_ortiz'


class ApproxOrtizChoquetGaussianMixture(ApproxOrtizGaussianMixture, ChoquetGaussianMixture):
    name = 'choquet_approx'
